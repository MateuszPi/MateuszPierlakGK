...
void GenerateTextures()
{
	...
	GLboolean error = load_targa("mixed_grass_2.tga", width, height, format, type, pixels);
	...
	error = load_targa("birch_tree_small.tga", width, height, format, type, pixels);

	// błąd odczytu pliku
	if (error == GL_FALSE)
	{
		printf("Niepoprawny odczyt pliku birch_tree_small.tga");
		exit(0);
	}
